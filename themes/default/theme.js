...
/* 文章内容 */
var detailTpl_content = '{{#post}}<main class="container"><div class="mdui-card post-body"><div class="mdui-card-content"><div class="mdui-typo post-content"><h1 class="post-heading">{{title}}</h1><span class="create_at"><a href="{{user.html_url}}">{{user.login}}</a> 发表于 {{formatTime(created_at)}}</span><div class="mdui-divider"></div><div class="content">{{{markdown2HTML(body)}}}</div></div></div></div></main>{{/post}}';
